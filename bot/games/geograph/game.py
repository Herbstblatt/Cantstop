...
async def start_game(game, ctx, rounds, continent):
	# Проверка континента
	if continent.title() not in data['continents']['general']:
	...
	if not rounds.isdigit():
		...
		return
	elif continent != 'Мир' and int(rounds) > len(data['continents']['old' if game == 'old' else 'general'][continent.title()]):
		await ctx.send(f"Число раундов не может быть больше {len(data['continents']['old' if game == 'old' else 'general'][continent.title()])}.")
		return
	...
	if games.get(ctx.channel.id):
		...
		return
	# Если всё правильно
	games[ctx.channel.id] = {'game': game, 'id': randint(1, 1000), 'count': 0, 'max_count': int(rounds), 'continent': continent.title(), 'answered': [], 'leaders': {'Я': 0}, 'actived': False}
	while not games[ctx.channel.id]['actived'] and games[ctx.channel.id]['count'] < games[ctx.channel.id]['max_count']:
		...
		if games[ctx.channel.id]['count'] >= games[ctx.channel.id]['max_count']:
			leaders = ''
			for k,v in games[ctx.channel.id]['leaders'].items():
				if k == 'Я': continue
				leaders += ', **' + ctx.guild.get_member(k).global_name + f'** ({v})'
			await ctx.send(f'Игра закончена. Статистика: **я** ({games[ctx.channel.id]["leaders"]["Я"]}){leaders}.')
...
async def capitals(channel):
	...
	await channel.send(embed=embed)
	await asyncio.sleep(15)
	if id == games[channel.id]['id'] and country not in games[channel.id]['answered']:
		games[channel.id]['leaders']['Я'] += 1
		games[channel.id]['answered'].append(country)
		await channel.send(f'Правильный ответ: **{games[channel.id]["answer"]}**')
		return False
...
class GeographCog(commands.Cog):
	@commands.Cog.listener()
	async def on_message(self, message):
		if message.channel.id in games and message.content.lower() in (games[message.channel.id]['answer'].lower(), getattr(data['aliases'].get(games[message.channel.id]['answer']), 'lower', lambda: None)()) and games[message.channel.id]['answer'] not in games[message.channel.id]['answered']:
			...
			while games[message.channel.id]['count'] < games[message.channel.id]['max_count']:
				...
				if games[message.channel.id]['count'] >= games[message.channel.id]['max_count']:
					leaders = ''
					for k,v in games[message.channel.id]['leaders'].items():
						if k == 'Я': continue
						leaders += ', **' + message.guild.get_member(k).global_name + f'** ({v})'
					await message.channel.send(f'Игра закончена. Статистика: **я** ({games[message.channel.id]["leaders"]["Я"]}){leaders}.')
