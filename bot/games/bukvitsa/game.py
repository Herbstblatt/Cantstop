...
def generate_letters(distribution: dict = LETTER_PROBABILITY_DISTRIBUTION) -> list:
    return random.choices(list(distribution.keys()), weights = list(distribution.values()), k = 3)

def check_word(letters: list, word: str, dictionary: set = DICTIONARY) -> bool:
    if all([letters.count(l) <= word.lower().count(l) for l in letters]) and word.lower() in DICTIONARY:
        return True
    return False

class Game(discord.ui.View): 
    def __init__(self, host: discord.User, channel: discord.TextChannel, g_list, timeout: int = 420):
    ...
    async def start(self, interaction: discord.Interaction, button: discord.Button):
        if interaction.user == self.host:
            ...
            view.add_item(discord.ui.Button(label = 'Игра начата', style = discord.ButtonStyle.grey, disabled = True))
            await interaction.message.edit(view = view)
            self.ongoing = True
            for player in self.players:
                try:
                    await player.send(embed = discord.Embed(description = f'Вы должны составить как можно больше слов с буквами **{"**, **".join(self.letters)}** за три минуты', colour = discord.Colour.blue()))
                except discord.errors.Forbidden:
                    await self.channel.send(embed = discord.Embed(title = ':exclamation: Ошибка', description = f'{player.mention}, бот не может отправлять вам сообщения в личные сообщения. Проверьте свои настройки конфиденциальности', colour = discord.Colour.red()))
            ...
            for player in self.players:
                try:
                    await player.send(embed = discord.Embed(description = f'Игра окончена', colour = discord.Colour.red()))
                except discord.errors.Forbidden:
                    pass
    ...
    async def leaderboard(self):
        leaderboard = ''
        for place, player in enumerate(sorted(self.players.items(), key = lambda player: len(player[1]), reverse = True)):
    ...
    async def update(self, response: discord.InteractionResponse):
        ...
        if len(self.players) > 1:
            content += '\n<:icons_Person:1009175664156487810>' + '\n<:icons_Person:1009175664156487810>'.join([player.mention for player in list(self.players.keys())[1:]])
        await response.edit_message(embed = discord.Embed(title = ':abc: Буквица', description = content, colour = discord.Colour.blue()))
...
class GameCog(commands.Cog):
    ...
    async def start(self, ctx: commands.Context):
        if self.g_list.check_user(ctx.author):
            ...
            await ctx.send(embed = discord.Embed(title = ':abc: Буквица', description = f'Игроки должны будут составить в личных сообщениях бота как можно больше слов c буквами **{"**, **".join(game.letters)}** за три минуты. Начать игру может только ведущий, нажав на соответствующую кнопку\n\n**Игроки**:\n<:icons_customstaff:1009175726810988714> {ctx.author.mention}', colour = discord.Colour.blue()), view = game)
    
    @commands.Cog.listener()
    async def on_message(self, msg):
        for game in self.g_list.games:
            if all([msg.author in set(game.players.keys()), type(msg.channel) == discord.DMChannel, game.ongoing]):
                if check_word(game.letters, msg.content) and msg.content.lower() not in game.players[msg.author]:
